...
async def list_systems(ctx: Context, limit: int = 50, offset: int = 0) -> Dict[str, Any]:
    ...
    systems = await _list_systems(token)
...
async def find_systems_by_name(
    ...
    systems = await _find_systems_by_name(name, token)
    normalized_limit, normalized_offset = normalize_pagination(limit=limit, offset=offset, default_limit=50, max_limit=500)
    paged_items, meta = paginate_items(systems, limit=normalized_limit, offset=normalized_offset)
    return {
        'items': paged_items,
...
async def find_systems_by_ip(
    ...
    systems = await _find_systems_by_ip(ip_address, token)
    normalized_limit, normalized_offset = normalize_pagination(limit=limit, offset=offset, default_limit=50, max_limit=500)
    paged_items, meta = paginate_items(systems, limit=normalized_limit, offset=normalized_offset)
    return {
        'items': paged_items,
...
async def _get_system_updates(
    ...
    async with _make_client() as client:
        ...
        if include_cves and paged_updates:
            ...
            cve_fetch_tasks = [
                _fetch_cves_for_erratum(client, update.get("advisory_name"), system_id, list_cves_api_path, token, ctx)
...
async def summarize_system_updates(
    ...
    """
    log_string = f"Getting update summary for system {system_identifier}"
    logger.info(log_string)
    await ctx.info(log_string)
...
async def query_system_updates(
    ...
    """
    log_string = f"Querying system updates for system {system_identifier}"
    logger.info(log_string)
    await ctx.info(log_string)
...
async def check_all_systems_for_updates(
    ...
    """
    log_string = f"Checking active systems for updates"
    logger.info(log_string)
    await ctx.info(log_string)
...
async def _check_all_systems_for_updates(
    ...
    paged_active_systems, active_scan_meta = paginate_items(
        active_systems,
        limit=normalized_limit,
        offset=normalized_offset,
    )

    msg = f"Checking {len(paged_active_systems)} systems for updates..."
    logger.info(msg)
    await ctx.info(msg)

    total_systems_to_scan = len(paged_active_systems)
    for i, system_summary in enumerate(paged_active_systems):
        system_id = system_summary.get('system_id')
        system_name = system_summary.get('system_name')

        await ctx.report_progress(i, total_systems_to_scan)
    ...
    if total_systems_to_scan > 0:
        await ctx.report_progress(total_systems_to_scan, total_systems_to_scan)
    ...
    meta = {
        **active_scan_meta,
        'returned_count': len(systems_with_updates),
    }
...
async def summarize_fleet_updates(
    ...
    """
    msg = f"Summarizing updates for active systems"
    logger.info(msg)
    await ctx.info(msg)
...
async def list_systems_needing_update_for_cve(
    ...
    token = await extract_token(ctx)
    return await _list_systems_needing_update_for_cve(cve_identifier, token, ctx, limit, offset)
...
async def list_systems_needing_reboot(
    ...
    return await _list_systems_needing_reboot(token, limit, offset)
...
async def _manage_group_systems(group_name: str, system_identifiers: List[str], add: bool, ctx: Context) -> str:
    ...
    resolved_sid_values = await asyncio.gather(
        *[_resolve_system_id(identifier, ctx, token) for identifier in system_identifiers]
    )
